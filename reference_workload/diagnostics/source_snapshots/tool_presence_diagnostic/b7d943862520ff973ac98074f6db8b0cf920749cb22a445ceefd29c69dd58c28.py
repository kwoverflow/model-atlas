...
def main() -> int:
    ...
    report = {
        ...
        "protocol": {
            ...
            "concurrency": 1,
            "expected_observations": len(cases) * 2 * entry.trials * len(VARIANTS),
            ...
            "changed": (
                "v2 second-stage optional-field presence prompt and public field checklist only"
            ),
            "unchanged": (
        ...
        for index, item in enumerate(cases):
            for order_index, order in enumerate(("registered", "reversed")):
                for trial in range(1, entry.trials + 1):
                    offset = (index * 4 + order_index * 2 + trial - 1) % len(VARIANTS)
    ...
    report["summaries"] = {
        v: summary([r for r in report["rows"] if r["variant"] == v]) for v in VARIANTS
    }
